...
class WeightedMatrix:
  ...
  def insert_values(self,dir_fuente=default_origin, dir_ques_fuente=  ,dir_txt_fuente=default_txt):
    ...
    for file_ in list_files:
      if file_ != ".DS_Store":
        ...
        f_cont = open(dir_txt_fuente + file_)
        word_list = f.read().split()
        content = f_cont.read()
        self.documents_content.append(content)
        self.number_of_words_in_document.append(int(len(self.word_list)))
    ...
    #print "DATO ORIGINAL"
    #print "number distinct word" + str(self.number_distinct_words)
    #self.number_distinct_words = int(len(self.word_list))
    #print "len(self.word_list)) = " + str(self.number_distinct_words)
    #exit(-1)
